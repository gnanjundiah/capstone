...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_QPSK_Transmit_v5_w_FIFO_process_check_sum_call( int
    nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_QPSK_Transmit_v5_w_FIFO_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_QPSK_Transmit_v5_w_FIFO_autoinheritance_info( int nlhs,
  ...
  if (strcmp(machineName, "QPSK_Transmit_v5_w_FIFO") == 0) {
    const mxArray *newRhs[3] = { NULL, NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_QPSK_Transmit_v5_w_FIFO_autoinheritance_info(nlhs,plhs,3,newRhs);
  }
...
{
  extern unsigned int sf_QPSK_Transmit_v5_w_FIFO_get_eml_resolved_functions_info
  ...
  if (strcmp(machineName, "QPSK_Transmit_v5_w_FIFO") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_QPSK_Transmit_v5_w_FIFO_get_eml_resolved_functions_info(nlhs,plhs,
      2,newRhs);
  }
...
extern unsigned int sf_QPSK_Transmit_v5_w_FIFO_method_dispatcher(SimStruct *S,
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"QPSK_Transmit_v5_w_FIFO")) {
    return(sf_QPSK_Transmit_v5_w_FIFO_method_dispatcher(simstructPtr,
...
extern void QPSK_Transmit_v5_w_FIFO_terminator(void);
void sf_machine_global_terminator(void)
{
  if (sfMachineGlobalTerminatorCallable) {
    ...
    QPSK_Transmit_v5_w_FIFO_terminator();
    sf_debug_terminate();
  }
...
extern void QPSK_Transmit_v5_w_FIFO_initializer(void);
extern void QPSK_Transmit_v5_w_FIFO_register_exported_symbols(SimStruct* S);
extern void QPSK_Transmit_v5_w_FIFO_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  QPSK_Transmit_v5_w_FIFO_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      QPSK_Transmit_v5_w_FIFO_debug_initialize();
    }

    QPSK_Transmit_v5_w_FIFO_initializer();
  }
